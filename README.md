...
- **已知问题**:
  1. `Dielectric` 材质应用在 `Box` 会失效，失效主要是指颜色会失效，无论如何设置颜色、密度、折射率，Box 都是透明无色的，排查过所有问题：每个面的法向量设置、hit 反转法向量、光线步进 delta、射线自交检测的容差精度等等，全都测试过，依然无效，暂时无法解决。
  2. `sponza` 场景虽然能渲染，但仍存在有一些问题，比如墙的纹理不太对、存在一些白噪点（这是因为场景较暗的地方，采光不足）。
  3. 偶然调整视角发现 `cube` 模型不知道为什么会缺少两个面。
