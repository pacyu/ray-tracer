...
float Cosine_pdf::value(const Vec3 &direction) const {
  ...
  return (cosine <= 0.0f) ? 0.0f : cosine / math::TRACER_PI;
}
...
Mixture_pdf::Mixture_pdf(std::shared_ptr<PDF> p0, std::shared_ptr<PDF> p1)
    : p0(std::move(p0)), p1(std::move(p1)), blend(0.5f) {}
...
float Mixture_pdf::value(const Vec3 &direction) const {
  if (!p0 || !p1)
    return 0.0f;
  return blend * p0->value(direction) + (1.0f - blend) * p1->value(direction);
}
...
Vec3 Hittable_pdf::generate() const {
  Vec3 p_light = ptr.random(o);
  Vec3 direction = p_light - o; // 得到从 O 指向光线表面的向量

  float d2 = direction.squared_length();
  if (d2 < 1e-6f)
...
float GGX_pdf::value(const Vec3 &direction) const {
  ...
  if (NoL < 1e-4f || NoV < 1e-4f || NoH < 1e-4f || VoH < 1e-4f)
    return 0.0f;
...
Vec3 Charlie_pdf::generate() const {
  ...
  float cos_theta = std::sqrt(std::max(0.0f, 1.0f - sin_theta * sin_theta));
...
float Charlie_pdf::value(const Vec3 &direction) const {
  ...
  float cos_theta_h = std::abs(dot(h, n));
  if (cos_theta_h <= 0.0f)
    return 0.0f;
  ...
  // 变换雅可比: p(l) = p(h) * cos(theta_h) / (4 * |v·h|)
  float p_l = (p_h * cos_theta_h) / (4.0f * v_dot_h);

  return std::max(p_l, 1e-6f);
}
