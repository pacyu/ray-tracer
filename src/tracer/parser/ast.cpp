...
DielectricNode::DielectricNode(std::shared_ptr<ASTNode> albedo,
    ...
    : albedo_expr(std::move(albedo)), density_expr(std::move(density)),
      ri_expr(std::move(ri)) {}

OceanMaterialNode::OceanMaterialNode(std::shared_ptr<ASTNode> ior,
                                     std::shared_ptr<ASTNode> roughness)
    : ior_expr(std::move(ior)), roughness_expr(std::move(roughness)) {}
...
OceanNode::OceanNode(std::shared_ptr<ASTNode> nx, std::shared_ptr<ASTNode> ny,
                     std::shared_ptr<ASTNode> lx, std::shared_ptr<ASTNode> ly,
                     std::shared_ptr<ASTNode> wind_speed,
                     std::shared_ptr<ASTNode> wind_dir,
                     std::shared_ptr<ASTNode> a, std::shared_ptr<ASTNode> mat,
                     std::shared_ptr<ASTNode> lam,
                     std::shared_ptr<ASTNode> height)
    : nx_expr(std::move(nx)), ny_expr(std::move(ny)), lx_expr(std::move(lx)),
      ly_expr(std::move(ly)), speed_expr(std::move(wind_speed)),
      dir_expr(std::move(wind_dir)), a_expr(std::move(a)),
      mat_expr(std::move(mat)), lambda_expr(std::move(lam)),
      height_expr(std::move(height)) {}
...
CameraNode::CameraNode(
    ...
    std::shared_ptr<ASTNode> depth, std::shared_ptr<ASTNode> name,
    std::shared_ptr<ASTNode> bg, std::shared_ptr<ASTNode> from,
    std::shared_ptr<ASTNode> at, std::shared_ptr<ASTNode> vup,
    std::shared_ptr<ASTNode> fov)
    : shape_expr(std::move(shape)), spp_expr(std::move(spp)),
      depth_expr(std::move(depth)), name_expr(std::move(name)),
      bg_expr(std::move(bg)), from_expr(std::move(from)),
      at_expr(std::move(at)), vup_expr(std::move(vup)),
      fov_expr(std::move(fov)) {}
...
BasicType LambertianNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  if (albedo.tag == BasicType::T_VEC3) {
    return BasicType(std::make_shared<material::Lambertian>(albedo.t_vector3));
  } else {
    return BasicType(std::make_shared<material::Lambertian>(
        std::make_shared<texture::ImageTexture>(albedo.t_string.c_str())));
  }
}
...
BasicType MetalNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  return BasicType(
      std::make_shared<material::Metal>(albedo.t_vector3, fuzz.t_float));
}
...
BasicType DielectricNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  if (density.tag == BasicType::T_INT)
    density.t_float = static_cast<float>(density.t_integer);
  if (ri.tag == BasicType::T_INT)
    ri.t_float = static_cast<float>(ri.t_integer);
  return BasicType(std::make_shared<material::Dielectric>(
      albedo.t_vector3, density.t_float, ri.t_float));
}
...
BasicType DiffuseLightNode::evaluate(std::shared_ptr<Environment> env) {
  BasicType albedo = albedo_expr->evaluate(env);
  return BasicType(std::make_shared<material::DiffuseLight>(albedo.t_vector3));
}
...
BasicType XYRectNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  return BasicType(
      std::make_shared<geometry::XYRect>(x, y, z, w, k, mat.t_material));
}
...
BasicType XZRectNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  float x = quater.t_quater.x, y = quater.t_quater.y, z = quater.t_quater.z,
        w = quater.t_quater.w, k = k_val.t_float;
  return BasicType(
      std::make_shared<geometry::XZRect>(x, y, z, w, k, mat.t_material));
}
...
BasicType YZRectNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  float x = quater.t_quater.x, y = quater.t_quater.y, z = quater.t_quater.z,
        w = quater.t_quater.w, k = k_val.t_float;
  return BasicType(
      std::make_shared<geometry::YZRect>(x, y, z, w, k, mat.t_material));
}
...
BasicType BoxNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  if (rot_expr) {
    ...
    if (x != 0.f)
      box = std::make_shared<transform::RotateX>(box, x);
    if (y != 0.f)
      box = std::make_shared<transform::RotateY>(box, y);
    if (z != 0.f)
...
BasicType HeartNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  if (rot_expr) {
    ...
    float z = rot.t_vector3.z();
    if (x != 0.f)
      heart = std::make_shared<transform::RotateX>(heart, x);
    if (y != 0.f)
      heart = std::make_shared<transform::RotateY>(heart, y);
    if (z != 0.f)
...
BasicType OceanNode::evaluate(std::shared_ptr<Environment> env) {
  BasicType nx = nx_expr->evaluate(env);
  BasicType ny = ny_expr->evaluate(env);
  BasicType lx = lx_expr->evaluate(env);
  BasicType ly = ly_expr->evaluate(env);
  BasicType wind_speed = speed_expr->evaluate(env);
  BasicType wind_dir = dir_expr->evaluate(env);
  BasicType a = a_expr->evaluate(env);
  BasicType mat = mat_expr->evaluate(env);
  BasicType lambda = lambda_expr->evaluate(env);
  BasicType height = height_expr->evaluate(env);
  int nx_ = static_cast<int>(nx.t_float);
  int ny_ = static_cast<int>(ny.t_float);
  physics::OceanParams params;
  params.Lx = lx.t_float;
  params.Ly = ly.t_float;
  ...
  params.A = a.t_float;
  auto simulator = std::make_unique<physics::FFTOcean>(nx_, ny_, params);
...
BasicType TranslateNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  return BasicType(std::make_shared<transform::Translate>(object.t_object,
                                                          offset.t_vector3));
}
...
BasicType CameraNode::evaluate(std::shared_ptr<Environment> env) {
  ...
  if (depth_expr)
    ...
    depth.t_integer = 8;
  if (name_expr)
    name = name_expr->evaluate(env);
  else
    name.t_string = "image.png";
  if (bg_expr)
