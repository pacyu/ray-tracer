...
bool Dielectric::scatter(const Ray &r_in, const hit_record &rec,
  ...
  if (!rec.front_face) {
    float distance = rec.t * 0.5f;
    // 计算指数衰减：exp(-density * distance)
    float r = std::exp(-density * (1.0f - albedo.r()) * distance);
    float g = std::exp(-density * (1.0f - albedo.g()) * distance);
    float b = std::exp(-density * (1.0f - albedo.b()) * distance);
  ...
  if (etai_over_etat * sin_theta > 1.f ||
    ...
    srec.specular_ray = Ray(rec.p + offset_epsilon * reflected, reflected);
  } else { // 折射
    Vec3 refracted =
        tracer::optics::refract(unit_direction, rec.normal, etai_over_etat);
    srec.specular_ray = Ray(rec.p + offset_epsilon * refracted, refracted);
  }
  return true;
}
